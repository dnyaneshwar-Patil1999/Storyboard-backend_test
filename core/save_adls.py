...
# Try to import the new markdown-aware renderer.
# If unavailable, save_adls falls back to the legacy add_text_or_table_or_image.
try:
...
def save_outline_to_adls(data: dict, filename: str = None):
    ...
    for outline in outlines:
        ...
        # Resolve the images list from the chunk so the renderer can
        ...
        if raw_content and raw_content.strip():
            if _CO_RENDERER_AVAILABLE:
                ...
                # Pass images list so [IMAGE: id] markers get resolved
                try:
                    render_content_into_cell(row_cells[3], raw_content, images=chunk_images)
                except Exception:
                    # Fallback to legacy renderer on any error
