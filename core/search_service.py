...
class BaseSearchAgent:
    ...
    def execute_keyword_search(self, query_text: str, active_filter: str, top_n=1000) -> List[dict]:
        ...
        try:
            results = self.search_client.search(
                ...
                # NEW CODE
                search_fields=["chapter", "topic", "subtopic", "content", "key_phrases"],
 ...
 # OLD CODE FOR HYBRIED SEARCH WITHOUT VECTORES: ONLY Keyword-based search (BM25)
...
# NEW CODE FOR HYBRIED SEARCH WITHOUT VECTORES

    def execute_hybrid_search(self, query_text, active_filter, top_n=5000):
        ...
        try:
            results = self.search_client.search(
                ...
                top=top_n,
                query_type="semantic",
                semantic_configuration_name="self-contained-semantic-config",
                select=["id", "content", "chapter", "topic", "subtopic", "source_page_range", "chunk_number", "File", "images", "table_images"],
            )
...
class OrchestratorAgent(ProcessesAgent):
    ...
    def save_results(self, results: list, metadata_inputs: dict, filename_prefix: str, output_filename: str = None, inject_template_rows: bool = False):
        ...
        inject_template_rows: If True (legacy behavior), prepends Welcome /
            Navigation Tour / Course Overview rows and appends Knowledge
            Check rows after each chapter. DEFAULT IS FALSE — reviewers
            consistently flagged these phantom rows.
        """
        ...
        if inject_template_rows:
            # LEGACY BEHAVIOR: inject phantom rows. Reviewers complained these rows are not in the source.
            enhanced_results = [
            ...
            for row in results:
                if last_chapter and row.get("Chapter") != last_chapter:
                    if "introduction" not in (last_chapter or "").lower():
            ...
            # DEFAULT: pass through source content without phantom rows
            enhanced_results = list(results)
            # If any result has an error, propagate it
